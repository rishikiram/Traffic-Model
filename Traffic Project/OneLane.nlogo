...
stoplights-own[up? down? right? left? switch maxswitch]
...
to make-roads
  ...
  make-intersections -25 0 25 0

  make-intersections -20 10 -20 -10
  make-intersections -10 -10 -10 10
  make-intersections 0 10 0 -10
  make-intersections 10 -10 10 10
  make-intersections 20 10 20 -10
...
to work-stoplights;not switching directions properly
  let hor ""
  ask stoplights
  [
    if switch <= 0 [print "switch"
      ask patch (xcor - 1) ycor [ set hor direction]

      if hor = "st" [if right? = true [switch-right xcor ycor set switch maxswitch]
        if left? = true [switch-left xcor ycor set switch maxswitch]
      ]
      if hor != "st" [if up? = true [switch-up xcor ycor set switch maxswitch]
        if down? = true [switch-down xcor ycor set switch maxswitch]
      ]
...
to switch-right [x y]
  ask patch (x + 1) y [set direction "rt"]
  ask patch (x - 1) y [set direction "rt"]
  ask patch x (y + 1) [set direction "st"]
  ask patch x (y - 1) [set direction "st"]
  ask patch x y [set pcolor red set direction "fd"]
end
to switch-left [x y]
  ask patch (x + 1) y [set direction "lt"]
  ask patch (x - 1) y [set direction "lt"]
  ask patch x (y + 1) [set direction "st"]
  ask patch x (y - 1) [set direction "st"]
  ask patch x y [set pcolor yellow set direction "fd"]
end
to switch-up [x y]
  ask patch (x + 1) y [set direction "st"]
  ask patch (x - 1) y [set direction "st"]
  ask patch x (y + 1) [set direction "up"]
  ask patch x (y - 1) [set direction "up"]
  ask patch x y [set pcolor green set direction "fd"]
end
to switch-down [x y]
  ask patch(x + 1) y [set direction "st"]
  ask patch (x - 1) y [set direction "st"]
  ask patch x (y + 1) [set direction "dn"]
  ask patch x (y - 1) [set direction "dn"]
  ask patch x y [set pcolor blue set direction "fd"]
end

to make-stoplights-square [h w trx try];height, width, top right x cor, top right y cor
...
to initialize-stoplights
  ...
  [
    ...
    ask  patch-at 0 1
    [
      if direction = "up" [ set u true]
      if direction = "dn" [set d true]
    ]
    ask  patch-at 1 0
    [
      if direction = "rt" [ set r true]
      if direction = "lt" [set l true]
    ]
    ...
    ask patch 0 0 [set direction "fd"]
  ]
end
...
to move-cars
  ...
  [
    ...
    while [i < spots and dir != "st"]; st means stop
    [
    if dir = "rt" and not any? cars-on patch-at 1 0;right
    [facexy (xcor + 1) ycor
        fd 1]
    if dir = "lt" and not any? cars-on patch-at -1 0;left
    [facexy (xcor - 1) ycor fd 1]
    if dir = "up" and not any? cars-on patch-at 0 1;up
    [facexy xcor (ycor + 1) fd 1]
    if dir = "dn" and not any? cars-on patch-at 0 -1;down
    [facexy xcor (ycor - 1) fd 1]
